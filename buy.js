function doit(generic, buy, build, advance, wood, stone, iron, cWood, cStone, cIron) {
    ...
    if (generic + buy < total) alert("This requires " + total + " RS. You do not have enough RS");
        ...
        window.location.href = ('action.html?GRS=' + generic + '&yellow=' + buy + '&red=' + build + '&blue=' + advance + '&wood=' + woodnum + '&stone=' + stonenum + '&iron=' + ironnum + '&board=' + sURLVariables[4].split('=')[1] + '&space=-1&turn=' + sURLVariables[6].split('=')[1] + '&players=' + sURLVariables[7].split('=')[1] + '&green=' + sURLVariables[8].split('=')[1] + "&required=" + sURLVariables[9].split('=')[1] + "&civNum=" + sURLVariables[10].split('=')[1]);
    }
}